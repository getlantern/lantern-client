...
extension PacketTunnelProvider {
    ...
    func startFlashlight(_ completionHandler: @escaping (Error?) -> Void) {
        flashlightManager.queue.async { [weak self] in
            ...
            var error: NSError?
            welf.client = IosClient(welf, UDPDialer(), MemChecker(), welf.constants.configDirectoryURL.path, welf.mtu, Constants.capturedDNSHost, Constants.realDNSHost, &error)
...
extension PacketTunnelProvider {
    ...
    private func loadExcludedRoutes() -> [NEIPv4Route] {
        ...
        var routes = [NEIPv4Route(destinationAddress: Constants.realDNSHost, subnetMask: "255.255.255.255")]  // dns
        ...
        do {
            ...
            logger.error("Loading excluded routes failed, killing tunnel process: \(error.localizedDescription)")
