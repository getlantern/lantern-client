...
struct Constants {
    ...
    init(process: Process,
        ...
        // Need refs for both on app-side for Log Submission
        self.appDirectoryURL =  sharedContainerURL.appendingPathComponent("app", isDirectory: true)
        self.netExDirectoryURL = sharedContainerURL.appendingPathComponent("netEx", isDirectory: true)
        print("app config path: \(configDirectoryURL)")
        
        let current: URL
        switch process {
        case .app: current = appDirectoryURL
        case .netEx: current = netExDirectoryURL
        }
        self.targetDirectoryURL = current
    }
    ...
    var goroutineProfileURL: URL { return targetDirectoryURL.appendingPathComponent("goroutine_profile.txt") }
    var goroutineProfileTempURL: URL { return targetDirectoryURL.appendingPathComponent("goroutine_profile.txt.tmp") }
    ...
    var proxyStatsTempURL: URL { return configDirectoryURL.appendingPathComponent("proxystats.csv.tmp")}
    var proxiesEtagURL: URL { return configDirectoryURL.appendingPathComponent("proxies.yaml.etag") }
    var tlsSessionStatesURL: URL { return configDirectoryURL.appendingPathComponent("tls_session_states")}
    var masqueradeCacheURL: URL { return configDirectoryURL.appendingPathComponent("masquerade_cache") }
    ...
    var allConfigURLs: [URL] {
        return [configURL, configEtagURL, proxiesURL, proxyStatsURL, proxyStatsTempURL, proxiesEtagURL, tlsSessionStatesURL, masqueradeCacheURL, userConfigURL, quotaURL, dnsgrabCacheURL]
    }
