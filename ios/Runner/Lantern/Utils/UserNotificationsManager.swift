...
class UserNotificationsManager: NSObject, UNUserNotificationCenterDelegate {
  ...
  private var notifiedDataCapThisMonth: Bool {
    guard
      let value = userDefaults.value(
        forKey: UserNotificationsManager.lastDataCapNotificationDefaultsKey),
      let lastDate = (value as? Date)
    else { return false }
  ...
  func localizedNotificationContent(withDataLimit limit: Int) -> UNMutableNotificationContent {
    ...
    if localeString == "zh_CN" {
      ...
      content.body =
        "You can continue to use Lantern, but speeds will be reduced. Your data cap will be reset at the beginning of the month."
    }
    return content
  }
}
