...
$(INSTALLER_NAME).dmg: require-version require-appdmg require-retry
	...
	if [[ "$$(uname -s)" == "Darwin" ]]; then \
		...
		cp -r build/macos/Build/Products/Release/androidlantern.app/Contents/* $(DARWIN_APP_NAME)/Contents/ && \
		...
		cat $(DARWIN_APP_NAME)/Contents/MacOS/androidlantern | bzip2 > $(APP)_update_darwin_amd64.bz2 && \
