...
open class PinField : AppCompatEditText {
    ...
    protected var distanceInBetween: Float = 0f
        set(value) {
            field = value
            requestLayout()
            invalidate()
    ...
    var fieldColor = ContextCompat.getColor(context, R.color.grey_4)
        set(value) {
            field = value
    ...
    var highlightPaintColor = ContextCompat.getColor(context, R.color.tertiary_green)
        set(value) {
            field = value
    ...
    private fun initParams(attr: AttributeSet) {
        ...
        try {
            ...
            singleFieldWidth = a.getDimension(R.styleable.PinField_singleFieldWidth, -1f).toInt()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        if (singleFieldWidth == -1) {
            singleFieldWidth = width / numberOfFields
        }
        val width = singleFieldWidth * numberOfFields + distanceInBetween * (numberOfFields - 1)
        setMeasuredDimension(width.toInt(), getViewHeight(singleFieldWidth, heightMeasureSpec))
    }
