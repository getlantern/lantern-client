...
public class ProxySettings {
    ...
    public static boolean setProxy(Context ctx, WebView webview,
        ...
        try {
            ...
            Logger.error(LOG_TAG, "error setting up webkit proxying", e);
        }
        return ret;
    }
    ...
    private static boolean setProxyJB(WebView webview, String host, int port) {
        Logger.debug(LOG_TAG, "Setting proxy with 4.1 - 4.3 API.");
        ...
        Logger.debug(LOG_TAG, "Setting proxy with 4.1 - 4.3 API successful!");
        return true;
    }

    // from https://stackoverflow.com/questions/19979578/android-webview-set-proxy-programatically-kitkat
    private static boolean setProxyKK(WebView webView, String host, int port) {
        Logger.debug(LOG_TAG, "Setting proxy with >= 4.4 API.");
        ...
        try {
            ...
            Logger.debug(LOG_TAG, "Setting proxy with >= 4.4 API successful!");
            return true;
        } catch (ClassNotFoundException e) {
            StringWriter sw = new StringWriter();
            e.printStackTrace(new PrintWriter(sw));
            String exceptionAsString = sw.toString();
            Logger.v(LOG_TAG, e.getMessage());
            Logger.v(LOG_TAG, exceptionAsString);
