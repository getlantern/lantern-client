...
public class LanternService extends Service implements Runnable {
    ...
    private final ServiceHelper helper = new ServiceHelper(
            this,
            LanternApp.getSession().chatEnabled() ? R.drawable.status_chat : R.drawable.status_plain,
...
//                    .equals(LanternApp.messaging.messaging.getDb().get("onBoardingStatus"));
    ...
    private void afterStart() {
        if (LanternApp.getSession().userId() == 0) {
            ...
            EventBus.getDefault().postSticky(new AccountInitializationStatus(AccountInitializationStatus.Status.PROCESSING));
            createUser(0);
        }
